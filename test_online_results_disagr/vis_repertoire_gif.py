...
v                        labels = [p for p in range(100)])
    ...
    if args.plot_type == "grid":
        ...
        #data.plot.scatter(x=1,y=2,c=3,colormap='viridis', s=2, ax=ax)
        ...
        plt.scatter(data.iloc[:,1], data.iloc[:,2], c=data.iloc[:,3], s=1)
        plt.clim(0,5)
        plt.colorbar()
        plt.xlim(0,1)
        plt.ylim(0,1)
...
if __name__ == "__main__":
    ...
    # build gif
    print('Creating gif\n')
    with imageio.get_writer('model_disagreement_emit_disagrplot.gif', mode='I') as writer:
