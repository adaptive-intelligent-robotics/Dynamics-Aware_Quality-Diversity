...
class A_star():
    def __init__(self, obstacles, boundary, resolution):
        ...
        # ADD COORDINATES OF BOTH OBSTACLES AND BOUNDARY TO self.obstacles[]
        # to check for collision later
        
        # GET ALL COORDINATES FOR THE OBSTACLES
        obsx, obsy = [], []
        ...
        # GET ALL COORDINATES FOR MAP BOUNDARIES
    ...
    def best_node(self, open_nodes, goal):
        best = object
        min_cost = np.finfo(float).max # positive machine limit infinity

        for key in open_nodes.keys():
            curr = np.array([open_nodes[key].x, open_nodes[key].y])

            # a* metric cost + heuristic (heuristic is distance to goal)
            # best node  is lowest cost
    ...
    def plan(self, start, goal):
        '''
        keys for open and closed nodes dict are tuples of points xy
        '''
        ...
        while not solved:
            ...
            # generate new nodes FROM CURRENT BEST NODE
